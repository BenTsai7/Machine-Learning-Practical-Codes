...
if __name__ == '__main__':
    ...
    # 打印字典信息
    # print(lenses_dict)
    # 生成pandas.DataFrame用于对象的创建，DataFrame类似于关系数据库的表
    lenses_pd = pd.DataFrame(lenses_dict)
    # 打印数据
    #print(lenses_pd)
    # 创建LabelEncoder对象
    le = LabelEncoder()
    # 为每一列序列化，实际上就是将字符串类特征转换为数值
    ...
    # 保存绘制好的决策树，以PDF的形式存储。需要安装GraphViz，否则会报错
    #graph.write_pdf("tree.pdf")
    #预测
    print(clf.predict([[1,1,1,0]]))
    rfc = RandomForestClassifier()
    rfc.fit(lenses_pd.values.tolist(), np.array(lenses_targt).ravel())
    print(rfc.predict([[1,1,1,0]]))
